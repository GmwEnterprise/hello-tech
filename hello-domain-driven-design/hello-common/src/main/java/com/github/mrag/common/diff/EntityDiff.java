...
public class EntityDiff<T extends Entity<ID>, ID extends Identifier<?>> implements Diff<T, ID> {
    ...
    EntityDiff(T entity, T snapshot) {
        if (entity == null && snapshot != null) {
            valueType = (Class<T>) ReflectUtils.unwrapAnonymous(snapshot.getClass());
            diffType = DiffType.Removed;
            oldValue = snapshot;
            newValue = null;
        } else if (entity != null && snapshot == null) {
            valueType = (Class<T>) ReflectUtils.unwrapAnonymous(entity.getClass());
    ...
    public String toString() {
        return String.format(
                "diffType = %s, valueType = %s, diffMap = %s",
                diffType.name(), valueType.getSimpleName(), diffMap);
    }
    ...
    private int calculate(T entity, T snapshot) {
        ...
        List<Field> allFields = ReflectUtils.getAllFields(valueType);
        for (Field member : allFields) {

            // 获取field的类型 memberType
            Class<?> memberType = member.getType();
            Object entityFieldRawValue = ReflectUtils.getFieldValue(member, entity);
            Object snapshotFieldRawValue = ReflectUtils.getFieldValue(member, snapshot);
            if (Entity.class.isAssignableFrom(memberType)) {
                // 嵌套实体
                diffMap.put(member.getName(), new EntityDiff<>(
                        ((Entity<Identifier<?>>) entityFieldRawValue),
                        ((Entity<Identifier<?>>) snapshotFieldRawValue)));
                continue;
            }
            if (Collection.class.isAssignableFrom(memberType)) {
                ...
                if (Entity.class.isAssignableFrom(genericType)) {
                    // 集合嵌套实体
                    Collection<Entity<Identifier<?>>> entityFieldValue = (Collection<Entity<Identifier<?>>>) entityFieldRawValue;
                    Collection<Entity<Identifier<?>>> snapshotFieldValue = (Collection<Entity<Identifier<?>>>) snapshotFieldRawValue;
            ...
            if (Map.class.isAssignableFrom(memberType)) {
                ...
                if (Entity.class.isAssignableFrom(genericType)) {
                    // 映射嵌套实体
                    Map<String, Entity<Identifier<?>>> entityFieldValue = (Map<String, Entity<Identifier<?>>>) entityFieldRawValue;
                    Map<String, Entity<Identifier<?>>> snapshotFieldValue = (Map<String, Entity<Identifier<?>>>) snapshotFieldRawValue;
            ...
            // 普通field，直接equals
            if (!CommonUtils.equals(entityFieldRawValue, snapshotFieldRawValue)) {
                selfModifiedCount++;
            }
        }
